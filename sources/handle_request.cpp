...
struct Ret HandleErrors(std::string errorType)
{
	...
	resp.response_headers.insert(
	    std::make_pair("Content-Length", std::to_string(contentLenght)));
...
size_t examineRoutes(std::vector<Location> locations, std::string path)
{
	...
	{
		if (path.find(locations[j].getPath()) != std::string::npos &&
		    locations[j].getPath().length() > priority)
			pathNum = j;
	}
	return pathNum;
}
	...
	else
	{
		for (i = data.size() - 1; i > 0; i--)
		{
			if (std::find(data[i].getNames().begin(), data[i].getNames().end(),
			              request.request_headers.find("Host")->second) !=
			    data[i].getNames().end())
			{
				break;
			}
		...
		response.response_headers["Content-Length"] =
		    std::to_string(contentLenght);
		if (request.request_headers.count("Connection") == 0 ||
		    response.response_headers["Content-Type"] == "Keep-alive")
...
struct Ret handleRequest(std::string buff, std::vector<ServerData> &data)
{
	...
	while ((token = getToken(buff, CRLF)) != "")
	{
		request.request_headers.insert(
		    std::make_pair(getToken(token, HeaderPairsDelim),
