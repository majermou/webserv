...
std::string const
    ConfigParser::primitives_openings[NUMBER_OF_SERVER_PRIMITIVES] = {
        PORT_OP,       HOST_OP, SERVER_NAME_OP, CLIENT_MAX_SIZE_BODY_OP,
        ERROR_PAGE_OP, ROOT_OP, LOCATION_OP,
};

std::string const
    ConfigParser::location_identifiers[NUMBER_OF_LOCATION_PRIMITIVES] = {
        LOC_ROOT,   LOC_AUTOINDEX, LOC_INDEX,         LOC_ALLOWED_METHODS,
        LOC_RETURN, LOC_CGI,       UPLOAD_LOC_ENABLE, UPLOAD_LOC_STORE,
};
...
void ConfigParser::_parseArguments(int ac, char *av[])
{
	if (ac > 2 || ac < 1 || av == NULL)
		throw std::runtime_error(
		    "Invalid arguments: USAGE [ ./webserv configFile.conf ]");
	if (ac == 2)
		_filename = av[1];
	else
...
ConfigParser::ConfigParser(int ac, char *av[])
{
	...
	if (DEBUG)
	{
		...
		{
			output << "start = [" << _serversIndexing[i] << "], end["
			       << _serversIndexing[i + 1] << "]" << std::endl;
		}
	}
	...
	outputLogs(std::string("configuration ") + _filename +
...
void ConfigParser::_trim(std::string &str)
{
	...
	while (!str.empty() && isspace(str.back())) str.erase(str.size() - 1, 1);
}
...
std::vector<std::string> ConfigParser::_split(std::string const &_line)
{
	...
	while (ss >> buff) wordsArr.push_back(buff);
	return wordsArr;
}
...
std::vector<std::string> ConfigParser::_split(std::string const &_line, char c)
{
	...
	while (getline(ss, buff, c)) wordsArr.push_back(buff);
	return wordsArr;
}
...
std::string const &ConfigParser::_removeDuplicateChar(std::string &str,
                                                      char const c)
{
...
void ConfigParser::_indexServers()
{
	...
	for (size_t i = 0; i < _fileLines.size(); i++)
	{
		...
		if (count > 1)
			...
			throw std::runtime_error(ERROR_BRACE_NOT_ALONE +
			                         getStringType("[") + _fileLines[i] + "]");

		if ((pos = _fileLines[i].find(OPENNING_BRACE)) != std::string::npos)
		{
			if (!i || (i > 0 && _fileLines[i - 1] != SERVER_OP &&
			           _fileLines[i - 1].compare(0, strlen(LOCATION_OP),
			                                     LOCATION_OP)))
				throw std::runtime_error(
				    ERROR_OPENING_BRACE_WITHOUT_SERVER_OR_LOC);
			isBracesValid++;
		}
		...
		if (_fileLines[i].compare(SERVER_OP) == 0)
		{
			...
			if (i + 1 < _fileLines.size() &&
			    _fileLines[i + 1].compare(OPENNING_BRACE) == 0)
			{
			...
			throw std::runtime_error(ERROR_INVALID_IDENTIFIER +
			                         getStringType("[") + _fileLines[i] + "]");
	}
...
static std::string convertStrArrToString(std::vector<std::string> const &arr,
...
void ConfigParser::_parseContent()
{
	...
	ParserFuncPtr _server_primitive_parser[NUMBER_OF_SERVER_PRIMITIVES] = {
	    &ConfigParser::_portParser,       &ConfigParser::_hostParser,
	    &ConfigParser::_serverNameParser, &ConfigParser::_clientBodySizeParser,
	    &ConfigParser::_errorPageParser,  &ConfigParser::_rootDirParser,
	    &ConfigParser::_locationParser,
	};

	for (size_t i = 0; i < NUMBER_OF_SERVER_PRIMITIVES; i++)
		_checked_primitives.insert(
		    std::pair<std::string, bool>(primitives_openings[i], false));
	for (size_t i = 0; i < NUMBER_OF_LOCATION_PRIMITIVES; i++)
		_checked_primitives.insert(
		    std::pair<std::string, bool>(location_identifiers[i], false));
	...
	while (i < _serversIndexing.size())
	{
		...
		while (start < end)
		{
			if ((parserIndex = _isPrimitive(_fileLines[start])) >= 0)
			{
				if (primitives_openings[parserIndex] != ERROR_PAGE_OP &&
					...
					throw std::runtime_error(ERROR_SERVER_DUPLICATE_FIELD +
					                         getStringType("[") +
					                         _fileLines[start] + "]");
				...
				if (static_cast<size_t>(
				        doneParsingIndex =
				            (this->*_server_primitive_parser[parserIndex])(
				                start, sv)) == end + 1)
					throw std::runtime_error(ERROR_INVALID_CONFIGURATION +
					                         getStringType("[") +
					                         _fileLines[start] + "]");
		...
		if (!missing_elements.empty())
			throw std::runtime_error(
			    std::to_string(missing_elements.size()) +
			    std::string(ERROR_MISSING_ELEMENTS) +
			    convertStrArrToString(missing_elements, ", ") +
			    " in the server N°" + std::to_string((i + 2) / 2) +
			    " configuration");
...
int ConfigParser::_isPrimitive(std::string const &_line)
{
	for (size_t i = 0; i < NUMBER_OF_SERVER_PRIMITIVES; i++)
	{
		if (_line.compare(0, primitives_openings[i].size(),
		                  primitives_openings[i]) == 0)
		{
			return i;
		}
	}
...
int ConfigParser::_isLocationPrimitive(std::string const &_line)
{
	for (size_t i = 0; i < NUMBER_OF_LOCATION_PRIMITIVES; i++)
	{
		if (_line.compare(0, location_identifiers[i].size(),
		                  location_identifiers[i]) == 0)
		{
			return i;
		}
	}
...
int ConfigParser::_portParser(size_t index, ServerData &sv)
{
	...
	{
		if (tokens[0] != PORT_OP)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(PORT_OP) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
		...
		throw std::runtime_error(ERROR_INVALID_CONFIGURATION +
		                         getStringType("[") + _fileLines[index] + "]");
...
int ConfigParser::_hostParser(size_t index, ServerData &sv)
{
	...
	{
		if (tokens[0] != HOST_OP)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(HOST_OP) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
...
int ConfigParser::_serverNameParser(size_t index, ServerData &sv)
{
	...
	if (tokens.size() >= 2)
	{
		if (tokens[0] != SERVER_NAME_OP)
			throw std::runtime_error(
			    DID_YOU_MEAN + getStringType(SERVER_NAME_OP) + IN_THIS_LINE +
			    "[" + _fileLines[index] + "] ?");
...
int ConfigParser::_clientBodySizeParser(size_t index, ServerData &sv)
{
	...
	if (tokens.size() == 2)
	{
		if (tokens[0] != CLIENT_MAX_SIZE_BODY_OP)
			throw std::runtime_error(
			    DID_YOU_MEAN + getStringType(CLIENT_MAX_SIZE_BODY_OP) +
			    IN_THIS_LINE + "[" + _fileLines[index] + "] ?");
		if (!_isSet(tokens[1], &(std::isdigit)))
...
int ConfigParser::_errorPageParser(size_t index, ServerData &sv)
{
	...
	if (tokens.size() == 3)
	{
		if (tokens[0] != ERROR_PAGE_OP)
			throw std::runtime_error(
			    DID_YOU_MEAN + getStringType(ERROR_PAGE_OP) + IN_THIS_LINE +
			    "[" + _fileLines[index] + "] ?");
		if (!_isSet(tokens[1], &(std::isdigit)))
			throw std::runtime_error(ERROR_ERRPAGE_CODE_NAN);
		sv.addErrorPage(std::atoi(tokens[1].c_str()),
		                _removeDuplicateChar(tokens[2], '/'));
	}
	else
		throw std::runtime_error(ERROR_INVALID_CONFIGURATION +
		                         getStringType("[") + _fileLines[index] + "]");
...
int ConfigParser::_rootDirParser(size_t index, ServerData &sv)
{
	...
	{
		if (tokens[0] != ROOT_OP)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(ROOT_OP) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
...
int ConfigParser::_locationParser(size_t index, ServerData &sv)
{
	if (_fileLines[index].back() == ';')
		throw std::runtime_error(ERROR_LOCATION_WITH_SEMICOLON +
		                         getStringType("[") + _fileLines[index] + "]");
	...
	LocationFieldParserFuncPtr
	    _location_primitive_parser[NUMBER_OF_LOCATION_PRIMITIVES] = {
	...
	while (_fileLines[index].compare(CLOSING_BRACE))
	{
		if ((parserIndex = _isLocationPrimitive(_fileLines[index])) >= 0)
		{
			if (_checked_location_primitives[location_identifiers[parserIndex]])
				throw std::runtime_error(ERROR_LOCATION_DUPLICATE_FIELD +
				                         getStringType("[") +
				                         _fileLines[index] + "]");
			(this->*_location_primitive_parser[parserIndex])(index, loc);
			_checked_location_primitives[location_identifiers[parserIndex]] =
			    true;
		}
		else
			throw std::runtime_error(ERROR_INVALID_CONFIGURATION +
...
void ConfigParser::_locationPathParser(size_t &index, Location &loc)
{
	...
	if (tokens.size() == 2)
	{
		if (tokens[0] != LOCATION_OP)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(LOCATION_OP) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
...
void ConfigParser::_locRootDirParser(size_t index, Location &loc)
{
	...
	if (tokens.size() == 2)
	{
		if (tokens[0] != LOC_ROOT)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(LOC_ROOT) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
...
void ConfigParser::_locAutoIndexParser(size_t index, Location &loc)
{
	...
	if (tokens.size() == 2)
	{
		if (tokens[0] != LOC_AUTOINDEX)
			throw std::runtime_error(
			    DID_YOU_MEAN + getStringType(LOC_AUTOINDEX) + IN_THIS_LINE +
			    "[" + _fileLines[index] + "] ?");
...
void ConfigParser::_locIndexParser(size_t index, Location &loc)
{
	...
	if (tokens[0] != LOC_INDEX)
		throw std::runtime_error(DID_YOU_MEAN + getStringType(LOC_INDEX) +
		                         IN_THIS_LINE + "[" + _fileLines[index] +
		                         "] ?");
	if (tokens.size() != 2)
		throw std::runtime_error(ERROR_INVALID_CONFIGURATION +
...
void ConfigParser::_locAllowedMethodsParser(size_t index, Location &loc)
{
	...
	if (buff != LOC_ALLOWED_METHODS)
		throw std::runtime_error(
		    DID_YOU_MEAN + getStringType(LOC_ALLOWED_METHODS) + IN_THIS_LINE +
		    "[" + _fileLines[index] + "] ?");
	...
	if (tokens.size() < 1 || tokens.size() > 3 ||
	    tokens.size() !=
	        (size_t)std::count(_line.begin(), _line.end(), ',') + 1)
		throw std::runtime_error(ERROR_ALLOWED_METHODS_SYNTAX +
		                         getStringType("[") + _fileLines[index] + "]");
...
void ConfigParser::_locUploadEnableParser(size_t index, Location &loc)
{
	...
	if (tokens.size() == 2)
	{
		if (tokens[0] != UPLOAD_LOC_ENABLE)
			throw std::runtime_error(
			    DID_YOU_MEAN + getStringType(UPLOAD_LOC_ENABLE) + IN_THIS_LINE +
			    "[" + _fileLines[index] + "] ?");
...
void ConfigParser::_locUploadLocationParser(size_t index, Location &loc)
{
	...
	if (tokens.size() == 2)
	{
		if (tokens[0] != UPLOAD_LOC_STORE)
			throw std::runtime_error(
			    DID_YOU_MEAN + getStringType(UPLOAD_LOC_STORE) + IN_THIS_LINE +
			    "[" + _fileLines[index] + "] ?");
...
void ConfigParser::_locRedirectionParser(size_t index, Location &loc)
{
	...
	if (tokens.size() == 3)
	{
		if (tokens[0] != LOC_RETURN)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(LOC_RETURN) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
		if (!_isSet(tokens[1], &(std::isdigit)))
...
void ConfigParser::_locCGIParser(size_t index, Location &loc)
{
	...
	{
		if (tokens[0] != LOC_CGI)
			throw std::runtime_error(DID_YOU_MEAN + getStringType(LOC_CGI) +
			                         IN_THIS_LINE + "[" + _fileLines[index] +
			                         "] ?");
		insideTokens = _split(loc.getPath(), '.');
		if (insideTokens.size() != 2 || insideTokens[0] != "*" ||
		    !_isSet(insideTokens[1], std::isalpha))
			throw std::runtime_error(ERROR_CGI_EXTENSION_ERROR +
			                         getStringType("[") + LOCATION_OP + " " +
			                         loc.getPath() + "]");
