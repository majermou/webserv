...
namespace SimpleWeb
{
	...
	public:
		                 ...
		                 public std::ostream
		{
			...
			void write_header(const CaseInsensitiveMultimap &header,
			                  size_type size)
			{
		...
		public:
			...
			/// Use this function if you need to recursively send parts of a
			/// longer message
			void send(const std::function<void(const error_code &)> &callback =
			              nullptr) noexcept
			{
				session->connection->set_timeout(timeout_content);
				auto self = this->shared_from_this();  // Keep Response instance
				                                       // alive through the
			...
			/// Convenience function for writing success status line, header
			/// fields, and content
			...
			/// If true, force server to close the connection after the response
			/// have been sent.
			///
			/// This is useful when implementing a HTTP/1.0-server sending
			/// content without specifying the content length.
		...
		class Request
		{
			...
			Request(std::size_t max_request_streambuf_size,
			    ...
			    noexcept : streambuf(max_request_streambuf_size),
			               content(streambuf),
		...
		public:
			...
			std::unique_ptr<socket_type>
			    socket;  // Socket must be unique_ptr since
			             // asio::ssl::stream<asio::ip::tcp::socket> is not
			             // movable
			...
			void close() noexcept
			{
				...
				std::unique_lock<std::mutex> lock(
				    socket_close_mutex);  // The following operations seems to
				                          // be needed to run sequentially
				...
				if (!this->connection->remote_endpoint)
				{
					...
					this->connection->remote_endpoint =
					    std::make_shared<asio::ip::tcp::endpoint>(
					        this->connection->socket->lowest_layer()
					            .remote_endpoint(ec));
				}
	...
	public:
		...
		public:
			...
			/// Set to 0 get an assigned port.
			unsigned short port;
			/// If io_service is not set, number of threads that the server will
			/// use when start() is called. Defaults to 1 thread.
			...
			/// IPv4 address in dotted decimal form or IPv6 address in
			/// hexadecimal notation. If empty, the address will be any address.
			std::string address;
	...
	public:
		...
		/// Returns assigned port. If io_service is not set, an internal
		/// io_service is created instead. Call before accept_and_run().
		unsigned short bind()
		{
			asio::ip::tcp::endpoint endpoint;
			if (config.address.size() > 0)
				endpoint = asio::ip::tcp::endpoint(
				    asio::ip::address::from_string(config.address),
				    config.port);
			else
				endpoint =
		...
		/// If you know the server port in advance, use start() instead.
		/// Accept requests, and if io_service was not set before calling
		/// bind(), run the internal io_service instead. Call after bind().
		void accept_and_run()
		{
			acceptor->listen();
			accept();
	...
	protected:
			    ...
			    *session->connection->socket, session->request->streambuf,
			                    ...
			                    std::size_t bytes_transferred)
			    {
				    ...
				    if (!ec)
				    {
					    ...
					    // as bytes_transferred, from Boost-docs: "After a
					    // successful async_read_until operation, the streambuf
					    // may contain additional data beyond the delimiter" The
					    // chosen solution is to extract lines from the stream
					    // directly when parsing the header. What is left of the
					    // streambuf (maybe some bytes of the content) is
					    // appended to in the async_read-function below (for
					    // retrieving content).
				    ...
				    if (!ec)
				    {
					    ...
					    if ((2 + length) > num_additional_bytes)
					    {
						    ...
						    asio::async_read(
						        *session->connection->socket,
						        session->request->streambuf,
						        asio::transfer_exactly(2 + length -
		    ...
		    unsigned long length)
		{
			...
			if (length > 0)
			{
				...
				session->request->content.read(
				    buffer.get(), static_cast<std::streamsize>(length));
				tmp_stream.write(buffer.get(),
		        ...
		        &resource_function)
		{
				    ...
				    response->send(
				        [this, response](const error_code &ec)
				        {
					        if (!ec)
					        {
						        ...
						        auto range = response->session->request->header
						                         .equal_range("Connection");
						        for (auto it = range.first; it != range.second;
						             it++)
						        {
							        if (case_insensitive_equal(it->second,
							                                   "close"))
								        return;
						        ...
						        if (response->session->request->http_version >=
						            "1.1")
						        {
							        auto new_session =
							            std::make_shared<Session>(
			...
			{
				if (on_error)
					on_error(session->request, make_error_code::make_error_code(
			...
			acceptor->async_accept(
			    *connection->socket,
			    [this, connection](const error_code &ec)
			    {
				    ...
				    // Immediately start accepting a new connection (unless
